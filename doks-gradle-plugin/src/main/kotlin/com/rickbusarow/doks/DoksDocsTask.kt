...
abstract class DoksDocsTask
  ...
  constructor(
     ...
     *
     * This property serves as a workaround for the limitations of incremental
     * tasks, which can't have the same inputs as their outputs. Since this task is
     * essentially a "formatting" task that modifies input files, we can't use the
     * actual input files as outputs without risking that they will be deleted by
     * Gradle in case of a binary change to the plugin or build environment. Instead,
     * we use this property to declare a separate directory as the output of this task.
     *
    ...
    private val autoCorrectProperty: Property<Boolean> =
      objects
    ...
    fun execute(inputChanges: InputChanges) {
      ...
      val resultsByRequest =
        samplesMapping
          ...
          ?.let { jsonString ->
            json.decodeFromString<Map<SampleRequest, SampleResult>>(jsonString)
          }.orEmpty()

      val resultsByRequestHash =
        resultsByRequest
